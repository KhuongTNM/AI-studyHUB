...
export function FlashcardPage() {
  ...
  // Trần số thẻ MỖI LƯỢT BẤM (BR-099) — độc lập với maxFlashcards (quota tổng).
  // Free luôn = 5; Premium là tham số cấu hình theo gói, LUÔN hữu hạn kể cả khi
  // maxFlashcards = -1 (không giới hạn tổng), để bảo vệ quota Gemini dùng chung.
  ...
  // was generated by AI or created manually.
  const totalCardCount = flashcards.length
  const remainingQuota = isUnlimited ? Infinity : Math.max(0, maxFlashcards - totalCardCount)
  // Số thẻ tối đa có thể NHẬP ở ô count: nhỏ hơn giữa quota tổng còn lại VÀ
  // trần per-click của gói (BR-099) — dù quota tổng còn nhiều (hoặc không giới
  ...
  const handleGenerateAI = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // 1 trong 4 lỗi MỚI của cơ chế Batching (per-click cap, timeout, rate-limit, safety) —
        // dùng mapErrorMessage() dùng chung, kèm số liệu thật cho per-click cap (Mục 6).
        toast.error(
        ...
        // Lỗi ĐÃ CÓ (quota tổng, document not found, chưa embedding xong, lỗi hệ thống...)
        // — hiển thị thẳng message như hành vi cũ, không switch/case (Mục 6).
          ...
          <div className="mb-6 grid gap-3 sm:grid-cols-[1fr_auto_auto]">
            ...
            <div className="rounded-2xl border border-border bg-background px-4 py-3 text-sm text-muted-foreground">
              {selectedDocId === "all"
                ...
                : remainingQuota <= 0
                ? text.quotaExhausted
                : (isUnlimited ? text.quotaUnlimited : text.quotaRemaining(remainingQuota, maxFlashcards))
